...
for chanName in channelList.keys():
    ...
    subprocess.call("bin/analysisMain.exe -c configs/"+chanName+"MCdataConf.txt -u -t -k "+str(channelList[chanName])+" --jetRegion 2,1,3,4 -v 16383 -z --dilepton --mvaDir mvaDirs/skims/met"+metStr+"mtw"+mtwStr + "/ --metCut " + str(metCut) + " --mtwCut " + str(mtwCut),shell=True)
