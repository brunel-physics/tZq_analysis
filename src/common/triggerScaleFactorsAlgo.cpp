...
void TriggerScaleFactors::runMainAnalysis()
{
    ...
    for (auto dataset = datasets.begin(); dataset != datasets.end(); ++dataset)
    {
        ...
        for (int i = 0; i < numberOfEvents; i++)
        {
            ...
            {
                triggerDoubleEG =
                    (doubleElectronTriggerCut(event, dataset->isMC()));
            }
            if (passDoubleElectronSelection)
            {
                triggerMetDoubleEG =
            ...
            {
                triggerDoubleMuon =
                    (doubleMuonTriggerCut(event, dataset->isMC()));
            }
            if (passDoubleMuonSelection)
            {
            ...
            {
                triggerMuonElectron =
                    (muonElectronTriggerCut(event, dataset->isMC()));
            }
            if (passMuonElectronSelection)
            {
                triggerMetMuonElectron =
...
std::vector<int> TriggerScaleFactors::getTightElectrons(AnalysisEvent* event)
{
    ...
    for (int i{0}; i < event->numElePF2PAT; i++)
    {
        ...
        if (!customElectronCuts_)
        { // Nominal cuts
            if (electrons.size() < 1 && tempVec.Pt() <= 35.)
            {
                continue;
            }
        ...
        { // Custom cuts of form minElectron1Pt maxElectron1Pt, minElectron2Pt,
          // maxElectron2Pt, minElectron1Eta maxElectron1Eta, minElectron2Eta,
          // maxElectron2Eta
            if (electrons.size() < 1 && tempVec.Pt() <= electronCutsVars[0])
            {
                continue;
            }
            if (electrons.size() < 1 && tempVec.Pt() > electronCutsVars[1]
                && electronCutsVars[1] > 0)
            {
                continue;
            }

            if (electrons.size() >= 1 && tempVec.Pt() <= electronCutsVars[2])
            {
                continue;
            }
            if (electrons.size() >= 1 && tempVec.Pt() > electronCutsVars[3]
                && electronCutsVars[3] > 0)
            {
                continue;
            }

            if (electrons.size() < 1
                && std::abs(event->elePF2PATSCEta[i]) <= electronCutsVars[4])
            {
                continue;
            }
            if (electrons.size() < 1
                && std::abs(event->elePF2PATSCEta[i]) > electronCutsVars[5])
            {
                continue;
            }

            if (electrons.size() >= 1
                && std::abs(event->elePF2PATSCEta[i]) <= electronCutsVars[6])
            {
                continue;
            }
            if (electrons.size() >= 1
                && std::abs(event->elePF2PATSCEta[i]) > electronCutsVars[7])
            {
                continue;
            }
        }

        if (std::abs(event->elePF2PATSCEta[i]) > 2.50)
        {
