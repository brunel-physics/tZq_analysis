...
double JetCorrectionUncertainty::getUncertainty ( double pt, double eta, int jesUD ) {

  if ( jesUD == 0 ) return 1.0;
  ...
  double b = (lowFact * ptMaxJEC_[ptBin] - highFact *ptMinJEC_[ptBin] ) / ( ptMaxJEC_[ptBin] - ptMinJEC_[ptBin] );

  return a*pt + b;

}

std::pair< double, double> JetCorrectionUncertainty::getMetAfterJESUnc( double metPx, double metPy, MvaEvent tree, int jesUD ) {

  for ( int i = 0; i != tree.numJetPF2PAT; i++ ) {
    metPx += tree.jetPF2PATPx[i];
    metPy += tree.jetPF2PATPy[i];    

    double uncertainty = getUncertainty ( tree.jetPF2PATPt[i],tree.jetPF2PATEta[i],jesUD );
